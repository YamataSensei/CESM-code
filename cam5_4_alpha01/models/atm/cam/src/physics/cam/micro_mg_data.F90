...
function pack_2D(self, unpacked) result(packed)
  ...
  real(r8) :: packed(self%mgncol,self%nlev+size(unpacked, 2)-self%pver)
...
subroutine MGFieldPostProc_process_and_unpack(self, packer)
  class(MGFieldPostProc), intent(in) :: self
...
! Use the same intent(out) trick, yet again.
subroutine MGPostProc_finalize(self)
  class(MGPostProc), intent(out) :: self
...
subroutine MGPostProc_process_and_unpack(self)
  class(MGPostProc), intent(in) :: self
