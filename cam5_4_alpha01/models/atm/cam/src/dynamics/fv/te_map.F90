...
! C.-C. Chen
   real(r8)  te_column(grid%ifirstxy:grid%ilastxy,grid%jfirstxy:grid%jlastxy)
!   real(r8)  t_correction
! C.-C. Chen
...
! C.-C. Chen, compute total column of TE (no phi_s*p_s - phi_t * p_t)
	do i=ifirst,ilast
        do j=jfirst,jlast
          te_column(i,j) = 0.0_r8
        do k=1,km
	  te_column(i,j) = te_column(i,j)+te(i,j,k)*(pe(i,k+1,j)-pe(i,k,j))
        enddo
        enddo
        enddo
! C.-C. Chen
