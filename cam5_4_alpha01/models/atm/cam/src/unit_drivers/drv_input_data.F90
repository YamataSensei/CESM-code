...
!=================================================================================
  subroutine drv_input_data_open( infile, indata )
    ...
    call cam_pio_openfile( indata%piofile, filen, PIO_NOCLOBBER)
    ...
    if ( .not. (data_dtime == dtime)) then
       write( iulog, * )  'drv_input_data_open: data mdt does not match dtime... use dtime = ', data_dtime
       call endrun( 'drv_input_data_open: data mdt does not match dtime.')
    endif
